...
显式文件输入保持严格识别；目录 / glob discovery 使用更宽容的发现规则，见下节。
...
- 目录 / glob discovery 输出 `SuiteResult`，即使只发现一个 testcase

---

## 6. Directory / Glob Discovery

CLI 可以接收目录和 glob pattern：
...
Discovery 是“批量收集 testcase 文件”，不是 suite 编排；不会引入目录级 env、setup 或 hooks。

发现规则：

- 目录输入会递归扫描 `.yaml`、`.yml`、`.json`
- glob pattern 由 Nextgen 展开，支持 `**`
- 发现结果按稳定路径顺序执行
- 发现结果按解析后的绝对路径去重
- 发现到的 testcase 文件会被组装成 name 为 `cli` 的 synthetic suite
- dry-run、tag filtering、reporter、`--output` 都复用现有 suite / 多文件链路

发现文件分类：

| 文件内容 | discovery 行为 |
|----------|----------------|
| 只有 `steps` | 收集为 testcase |
| 只有 `tests` | warning 并跳过；suite 文件必须显式传入 |
| 同时包含 `steps` 和 `tests` | 报错 |
| 二者都没有 | 忽略，便于 env/example YAML 与 testcase 同目录 |

错误语义：

- glob 无匹配时报错
- 目录或 glob 发现后没有任何 testcase 时报错
- 被发现 testcase 的解析、DAG 校验或执行错误沿用现有 suite / dry-run 语义
- 显式传入的单文件仍然严格分类；例如 `nextgen env/base.yaml` 会报无法识别，而 `nextgen tests/` 扫到 `env/base.yaml` 会忽略
...
# 发现目录 / glob 下的 testcase 文件，输出 SuiteResult
nextgen tests/
nextgen "tests/**/*.yaml"
```
